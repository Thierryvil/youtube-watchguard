...
const mockVideosClass: youtube_v3.Resource$Videos = {
  list: mockVideosList,
} as any;

const mockYoutubeAPI: youtube_v3.Youtube = {
  videos: mockVideosClass,
} as any;
...
const server = setupServer(
  rest.get('https://youtube.googleapis.com/youtube/v3/videos', (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(mockVideoResponse));
  })
);
...
describe('Search Route Integration Tests', () => {
  ...
  const makeSearchRequest = async (body: any) => {
